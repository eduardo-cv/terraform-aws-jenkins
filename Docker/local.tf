...
##########################################################
locals {
  index_psd = [for i in range(local.ec2_docker.count) : i % length(module.vpc.public_subnets[*]) + 1]
}
##########################################################
...
locals {
  pub_subnet_ids = {
    docker = [for i in local.index_psd : element(module.vpc.public_subnets[*], i - 1)]
  }
}
##########################################################