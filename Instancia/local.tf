...
##########################################################
locals {
  index_psj = [for i in range(local.ec2_jenkins.count) : i % length(module.vpc.public_subnets[*]) + 1]
}
##########################################################
...
locals {
  pub_subnet_ids = {
    jenkins = [for i in local.index_psj : element(module.vpc.public_subnets[*], i - 1)]
  }
}
##########################################################